...
public class AspectUtility : MonoBehaviour
{
    ...
    public float _wantedAspectRatio = 1.78f;
    public Color _camColor = Color.black;
    static float wantedAspectRatio = 1.78f;
    static Color camCol = Color.black;
    static Camera cam;
    ...
    public static void SetCamera(ref Camera cam)
    {
        float currentAspectRatio = (float)Screen.width / Screen.height;
        // If the current aspect ratio is already approximately equal to the desired aspect ratio,
        // use a full-screen Rect (in case it was set to something else previously)
    ...
    public static void BackGroundCam()
    {
        if (backgroundCam)
        {
            Destroy(backgroundCam.gameObject);
