...
public class StudioPanelUI : BasePanel, IListener
{
    ...
    {
        EventCenter.GetInstance().RemoveEventListener(EventData.ShowStudioUI, ShowUI);
        ...
        EventCenter.GetInstance().AddEventListener(EventData.ShowStudioUI, ShowUI);
    }
    ...
    {
        EventCenter.GetInstance().EventTrigger(EventData.HideStudioUI);
        RemoveListener();
        isButton = true;
        if (gameObject.activeSelf)
        {
